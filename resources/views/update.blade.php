...
@php
    ...
    // Capture the current user at the very start
...
@if ((auth()->user()->role == 'admin' && $Vgit > $Vlocal) || $isBeta)
    ...
    @if (empty($_SERVER['QUERY_STRING']))
        ...
        @if ($isBeta)
            ...
            <p>{{ __('messages.Installed beta version') }} = {{ file_exists(base_path('vbeta.json')) ? file_get_contents(base_path('vbeta.json')) : __('messages.none') }}</p>
            <p>{{ $Vgit > $Vlocal ? __('messages.You need to update to the latest mainline release') : __('messages.You’re running the latest mainline release') }}</p>
        @else
        ...
        <br>
        <div class="row">
            <a class="btn" href="{{ url()->current() }}/?{{ env('SKIP_UPDATE_BACKUP') == true ? 'preparing' : 'backup' }}">
                <button><i class="fa-solid fa-user-gear btn"></i> {{ __('messages.Update automatically') }}</button>
            </a>
    ...
    @if ($_SERVER['QUERY_STRING'] === 'updating')
        ...
        @php
            ...
            try {
                ...
                if ($response->failed()) throw new Exception("HTTP request failed: {$response->status()}");

                $zipPath = storage_path('update.zip');
                file_put_contents($zipPath, $response->body());

                $zip = new ZipArchive();
                if ($zip->open($zipPath) !== true) throw new Exception('Failed to open ZIP archive');
                $zip->extractTo(base_path());
                $zip->close();
                unlink($zipPath);

                // Re-login user after files replacement
        ...
        @if (session()->has('update_error'))
            <script>
                setTimeout(() => { window.location.href = "{{ url()->current() }}/?error"; }, 1000);
            </script>
        @else
            <script>
                fetch("{{ url('/') }}", { credentials: 'include' })
                    .then(r => r.ok ? window.location.href="{{ url()->current() }}?finishing" : window.location.href="{{ url('login') }}")
    ...
    @if ($_SERVER['QUERY_STRING'] === 'backups')
        @php
            set_time_limit(0);
            try { Artisan::call('backup:clean', ['--disable-notifications' => true]); } catch (Exception $e) { session(['update_error'=>$e->getMessage()]); }
            try { Artisan::call('backup:run', ['--only-files'=>true,'--disable-notifications'=>true]); } catch (Exception $e) { session(['update_error'=>$e->getMessage()]); }
        @endphp

        @if (session()->has('update_error'))
            <script>setTimeout(()=>{window.location.href="{{ url()->current() }}/?error";},1000);</script>
        @else
            @php file_put_contents(base_path('backups/CANUPDATE'), ''); @endphp
            <script>setTimeout(()=>{window.location.href="{{ url()->current() }}?preparing";},1000);</script>
        @endif
    @endif
    ...
    @if ($_SERVER['QUERY_STRING'] === 'preparing')
        ...
        @php
            set_time_limit(0);
            if (file_exists(storage_path('update.zip'))) { unlink(storage_path('update.zip')); }
        ...
        @else
            ...
            <script>setTimeout(()=>{window.location.href="{{ url()->current() }}?updating";},1000);</script>
        @endif
    @endif
...
{{-- Remaining sections (finishing, success, error) can use the same JS redirect pattern if needed --}}
{{-- Example: finishing --}}
@if ($_SERVER['QUERY_STRING'] === 'finishing')
    ...
    @if(!session()->has('update_error') && ($isBeta || $Vgit === $Vlocal))
        <script>window.location.href="{{ url()->current() }}?success";</script>
    @else
        @php if (!session()->has('update_error')) session(['update_error' => 'Update failed unexpectedly. Please try again later.']); @endphp
        <script>window.location.href="{{ url()->current() }}?error";</script>
    @endif
@endif
